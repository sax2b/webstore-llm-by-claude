...
| :--- | :--- | :--- |
| `order.awaiting_payment_title` | `"Press 'Pay Now' to Make a Payment"` | `"กดปุ่ม 'จ่ายเงิน' เพื่อชำระเงิน"` |
...
*   **Loading state**: Render `<LoadingIndicator />` while fetching order data using `GetOrder()` (`src/api/api.ts`).
*   **Layout**:
    *   **Header**: Render a centered text element displaying the localized message string depending on order state:
    | Order State | Translation Key | Note |
    | :--- | :--- | :--- |
    | `awaiting_payment` | `order.awaiting_payment_title`| `payment_url` must exists in order. If not, display a text with translation key`order.default_title` |
    ...
    *   **Group of Buttons**:
        *   Implement a fixed/sticky container panel at the baseline boundary of the screen. Inside this panel, render centered buttons horizontally below:
        *   **Return to Store Button**:
            *   Render a button element labeled via `translations['order.return_to_store_btn']`.
            *   On user click interaction, redirect to `/`.
        *   **Pay Now Button**:
            *   Render a button element labeled via `translations['order.pay_now_btn']` with primary color.
            *   On user click interaction, open a new tab via `payment_url` inside order data.
            *   **Condition**: The button appears if `order.state` is `awaiting_payment` and `order.payment_url` exists.
*   **Layout Constraints**:
...
### 6. Order Page Assembly(`src/pages/Order.ts`)
...
*  Consider query parameters in the path:
    ...
    *   If not, render Order Search View following the layout below:
        ...
        *   **View Order Button**:
            *   Render a centered button element (`w-full`) labeled via `translations['order.search_btn']`.
            *   On user submission, redirect to `/complete?e=<email>&o=<order_number>`.

### 7. Global Persistent Footer Menu (`src/components/BottomNavigation.tsx`)
