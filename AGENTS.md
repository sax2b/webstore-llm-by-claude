...
- **Runtime Verification:** Spin up the local development engine to verify that the application servers successfully without client-side exceptions:
  ...
  npm run dev
  ```
- **Manifest Handshake**: Once the application passes both validation checks, update tasks/manifest.json by changing the completed task's status parameter cleanly to "completed".