...
- Integrate the pre-existing Landing Page layout with products.

### 4. Syncronize Product Grid Data (`src/components/ProductGrid.tsx`)
- Modify an event handler when user selects product cart, load product full detail using `GetProduct()` in `src/api/api.ts` and use it to render product modal.
- Modify an event handler when user click add-to-cart button, use `UpdateCartContents()` to update cart contents. If success, fetch the current cart and re-render notification badge of cart button in the header (`src/components/Header.tsx`)`
