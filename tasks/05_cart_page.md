...
| Translation Key | English Value | Thai Value | Layout Target / Description |
| :--- | :--- | :--- | :--- |
| `cart.title` | `"My Cart"` | `"ตะกร้าสินค้าของฉัน"` | Primary main header label |
| `cart.price_summary` | `"Summary"` | `"สรุปรายการ"` | Summary divider card header |
| `cart.subtotal` | `"Subtotal"` | `"ยอดรวม"` | Baseline cost before taxes/shipping |
| `cart.vat` | `"VAT"` | `"ภาษีมูลค่าเพิ่ม"` | VAT |
| `cart.total` | `"Total"` | `"ยอดเงินรวมทั้งสิ้น"` | Final calculation absolute row |
| `cart.checkout` | `"Checkout"` | `"สั่งสินค้า"` | Blue mobile-optimized footer button |
| `cart.empty` | `"Your cart is empty"` | `"ไม่มีสินค้าในตะกร้า"` | Centered placeholder message when item count is 0 |
| `nav.shop`| `"Shop"` | `"ร้านค้า"` | Bottom navigation tab for landing page |
| `nav.cart`| `"Cart"` | `"ตะกร้าสินค้า"` | Bottom navigation tab for cart page |
...
- **Loading State**: Display `<LoadingIndicator/>` component from `src/components/LoadingIndicator.tsx` while the product list is fetching by using `GetCart()` (`src/api/api.ts`).
- **Header**: Render a text element displaying the localized message string from `translations['cart.title']`.
...
- **Checkout Button**:
    - Render a persistent full-width action element selector (`w-full`) labeled via `translations['cart.checkout']`.
