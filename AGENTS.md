...
- **4. Task Verification Protocol**: Once code implementation is complete, scroll to the Task Verification Requirements section at the bottom of the active task file. You must execute all file validations and terminal checks exactly as listed there.
- **5. Manifest Handshake & Loop**:
  - **FAILURE**: If any validation or terminal check inside the task verification section fails, immediately fix the errors in your source code, preserve codebase integrity, and restart the verification checklist from the linting step. Do not modify the manifest file while errors exist.
