...
*   **Go Back Button**: Render Go-Back button with symbol and a text labeled via `translations['go_back_btn']` to redirect to the previous page located on the top-left corner in any viewports.
*   **Header**: Render a left-aligned text element displaying the localized message string from `translations['checkout.title']` in any viewports.
*   **Address Section**: Use primary background in `BRANDING.md` to highlight this section
    ...
    *   **Information Section**: Implement consecutive text behind a related symbol with labels placed directly above each input. The fields must be:
        *   Row 1:
...
*   **Place Order Button**:
    *   Render a button labeled via `translations['checkout.place_order_btn']`.
...
*   **Desktop viewport**:
    *   Component must be splited into two distinct horizontal sides. The left side vertically stacks information section and stacked cart item section. The right side vertically stacks shipping configurations section, payment configurations section, price summary panel, and place order button.
*   **Mobile viewport**:
    *   Stacks information section, stacked cart item section, shipping configurations section, and payment configurations section, respectively.
    *   Render a persistent full-wide space containing place order button (`w-full`).
*   **Layout Constraints**:
...
*   **Header**: Render a left-aligned text element displaying the localized message string from `translations['shipping_info.title']` in any viewports.
*   **Customer Detail Section**:
    ...
    *   **Form Section**: Implement three consecutive text input fields with labels placed directly above each input. The fields must be:
        ...
        *   email (Type: email, Label: `translations['shipping_info.customer_detail.email']`).
*   **Save Button**:
    *   Render a button labeled via `translations['shipping_info.save_btn']`.
    *   On user submission, update `billing_address` using `UpdateCart()` (`src/api/api.ts`).
*   **Default**: Stack customer detail section and save button, respectively in all viewports.
*   **Mobile viewport**: Render a persistent full-wide space containing save button (`w-full`).
*   **Layout Constraints**:
    *   Disappear Persistent Footer Menu in this subview.
    *   All fixed/static elements at the baseline boundary MUST not overlap other elements.
...
*   On user click interaction, execute following below:
    *   **Loading State**: Render `<LoadingIndicator />` on the checkout button from `src/components/LoadingIndicator.tsx` while fetching data used by using APIs (`src/api/api.ts`):
